...
class RecoveryManager:
        ...
        try:
            if p_provider == "cryptopay" and EnvKeys.CRYPTO_PAY_TOKEN:
                ...
                if info.get("status") == "paid":
                    ...
                    if success:
                        await self._mark_payment_completed(p_id)
    ...
    async def _mark_payment_completed(self, payment_id: int):
        """Mark payment as completed after successful recovery."""
        from bot.database import Database
        from bot.database.models import Payments

        async with Database().session() as s:
            await s.execute(
                update(Payments).where(Payments.id == payment_id).values(status="completed")
            )
            await s.commit()
