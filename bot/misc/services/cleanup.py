        ...
        while self.running:
            ...
            try:
                ...
                async with Database().session() as s:
                    # 1. Delete old audit_log entries in batches (avoid DB lock)
                    audit_deleted = 0
                    while True:
                        batch = await s.execute(
                            delete(AuditLog).where(AuditLog.timestamp < audit_cutoff).limit(5000)
                        )
                        await s.commit()
                        n = batch.rowcount
                        audit_deleted += n
                    ...
                    # 2. Delete old pending/failed payments in batches
                    payments_deleted = 0
                    while True:
                        batch = await s.execute(
