...
from bot.middleware.auto_answer import AutoAnswerMiddleware
...
async def __on_start_up(dp: Dispatcher, bot: Bot) -> None:
    ...
    # Initialize invite rewards system (creates tables, sets channel ID)
    from bot.misc.invite_rewards import init_invite_rewards, init_db as init_invite_db
    import os
    channel_id = int(os.getenv("CHANNEL_ID", "0") or 0)
    if channel_id:
        init_invite_rewards(channel_id)
        await init_invite_db()
        # Verify bot is admin in channel (can check membership)
        try:
            me = await bot.get_me()
            member = await bot.get_chat_member(chat_id=channel_id, user_id=me.id)
            if member.status not in ("administrator", "creator"):
                logging.warning(
                    f"CHANNEL_ID={channel_id}: bot is NOT admin (status={member.status}). "
                    f"Invite verification will fail! Add @clerkstore_bot as admin to the channel."
                )
            else:
                logging.info(f"Invite rewards system initialized (channel={channel_id}, bot is admin ✅)")
        except Exception as e:
            logging.warning(
                f"CHANNEL_ID={channel_id}: cannot check channel membership: {e}. "
                f"Add @clerkstore_bot as admin to the channel."
            )
    else:
        logging.warning("CHANNEL_ID not set — invite rewards disabled")
    ...
    # Auto-answer all callbacks (prevents Telegram 'clock' animation)
    ...
    # Start the order reconciler — keeps Tipzy order statuses in sync and
    # issues refunds for failed orders. Survives restarts, unlike the
    # per-order in-memory watcher.
    global order_reconciler
    from bot.misc.services.order_reconciler import OrderReconciler
    order_reconciler = OrderReconciler(bot)
...
async def __on_shutdown(dp: Dispatcher, bot: Bot) -> None:
    """Initialize bot shutdown with timeouts to prevent hanging."""
    global recovery_manager, cleanup_manager, admin_server, webhook_active, order_reconciler
    ...
    # Recovery Manager Stop (with timeout)
    if recovery_manager:
        try:
            await asyncio.wait_for(recovery_manager.stop(), timeout=SHUTDOWN_TIMEOUT)
        except asyncio.TimeoutError:
            logging.warning("Recovery manager shutdown timed out")

    # Cleanup Manager Stop (with timeout)
    if cleanup_manager:
        try:
            await asyncio.wait_for(cleanup_manager.stop(), timeout=SHUTDOWN_TIMEOUT)
        except asyncio.TimeoutError:
            logging.warning("Cleanup manager shutdown timed out")

    # Order Reconciler Stop (with timeout)
    if order_reconciler:
        try:
            await asyncio.wait_for(order_reconciler.stop(), timeout=SHUTDOWN_TIMEOUT)
        except asyncio.TimeoutError:
            logging.warning("Order reconciler shutdown timed out")
    ...
    # Close CryptoPay shared HTTP session (with timeout)
    try:
        from bot.misc.services.payment import CryptoPayAPI
        await asyncio.wait_for(CryptoPayAPI.close_session(), timeout=5)
    except (Exception, asyncio.TimeoutError):
        pass

    # Close database engine (with timeout)
    try:
        await asyncio.wait_for(_Database().dispose(), timeout=5)
    except (Exception, asyncio.TimeoutError):
        logging.warning("Database dispose timed out")
    ...
    async with Bot(
        ...
        try:
            ...
            if EnvKeys.WEBHOOK_ENABLED == "1" and EnvKeys.WEBHOOK_URL:
                logging.info("DEBUG: Entering webhook mode")
                # Webhook mode
                ...
                # Polling mode
                logging.info("DEBUG: Entering polling mode")
                logging.info("DEBUG: About to call start_polling")
