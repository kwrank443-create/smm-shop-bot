"""FastAPI admin panel + API + Freemodel proxy."""
import asyncio
import logging
import time
...
from bot.web.api_admin import router as api_admin_router

logger = logging.getLogger(__name__)


class LoginRateLimiter:
    """Redis-backed rate limiter for login attempts by IP (with in-memory fallback)."""

    def __init__(self, max_attempts: int = 5, lockout_seconds: int = 900):
        self.max_attempts = max_attempts
        self.lockout_seconds = lockout_seconds
        self._prefix = "login_attempts:"
        self._redis = None
    ...
    def is_blocked(self, ip: str) -> bool:
        ...
        return len(self._memory_store.get(ip, [])) >= self.max_attempts
    ...
    def record_failure(self, ip: str) -> None:
        ...
        if r is not None:
            try:
                ...
                now = time.time()
                r.lpush(key, str(now))
                r.ltrim(key, 0, self.max_attempts * 2)
                r.expire(key, self.lockout_seconds)
                return
            except Exception:
                pass
        # In-memory fallback
    ...
    def reset(self, ip: str) -> None:
        ...
        if r is not None:
            try:
                r.delete(f"{self._prefix}{ip}")
            except Exception:
                pass
        self._memory_store.pop(ip, None)
...
from bot.database.models.main import (
    ...
    AuditLog, PromoCodes, CartItems, Reviews, TipzyOrder,
)
...
def create_admin_app() -> FastAPI:
    """Create FastAPI app with admin panel, API, and freemodel proxy."""
    
    # Create FastAPI app
    app = FastAPI(
        title="SMM Bot Admin API",
        description="Admin panel for SMM Telegram bot",
        version="1.0.0",
    )
    
    # Add middleware (SessionMiddleware from Starlette works with FastAPI)
    ...
    # Include API admin router
    app.include_router(api_admin_router, prefix="/api/admin")    
    # ─── Platega webhook routes ────────────────────────────────────────
    from bot.web.platega import platega_routes
    app.routes.extend(platega_routes)
    
    # ─── Freemodel admin routes ────────────────────────────────────────
    from bot.web.freemodel_admin import router as fm_router, init_tables

    app.include_router(fm_router, prefix="/freemodel")
    
    # Initialize tables
    try:
        asyncio.get_running_loop().create_task(init_tables())
    except RuntimeError:
        # Fallback if no running loop (shouldn't happen in production)
        try:
            asyncio.run(init_tables())
        except Exception as e:
            logger.warning(f"Could not init freemodel tables: {e}")
    except Exception as e:
        logger.warning(f"Could not init freemodel tables: {e}")

    # ─── Proxy /v1/* to freemodel proxy ──────────────────────────────────
    @app.api_route("/v1/{path:path}", methods=["GET", "POST", "PUT", "DELETE", "PATCH", "OPTIONS"])
    async def proxy_freemodel(request: Request, path: str):
        """Proxy all /v1/* requests to the freemodel proxy service."""
        # Require API key for /v1/* proxy
        ...
        async with httpx.AsyncClient(timeout=120.0) as client:
            url = f"http://127.0.0.1:8765/v1/{path}"
    ...
    @app.get("/health")
    async def health():
        return {"status": "ok", "service": "smm-bot-api"}
