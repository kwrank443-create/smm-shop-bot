...
from sqlalchemy import (
    ...
    DateTime, Numeric, Index, UniqueConstraint, CheckConstraint, func, select, JSON
)
...
class User(Database.BASE):
    ...
    language = Column(String(8), default='ru', nullable=False, server_default='ru')
...
class TipzyOrder(Database.BASE):
    """Tracks an order sent to TipzySMM API for a bot-start service."""
    __tablename__ = 'tipzy_orders'
    id = Column(Integer, primary_key=True)
    user_id = Column(BigInteger, ForeignKey('users.telegram_id', ondelete='SET NULL'), nullable=True, index=True)
    link = Column(Text, nullable=False)
    quantity = Column(Integer, nullable=False)
    price_paid = Column(Numeric(12, 2), nullable=False)
    tipzy_order_id = Column(String(64), nullable=True, index=True)
    status = Column(String(32), nullable=False, default='pending', index=True)
    tipzy_status = Column(String(32), nullable=True)
    tipzy_start_count = Column(Integer, nullable=True)
    tipzy_remains = Column(Integer, nullable=True)
    tipzy_charge = Column(Numeric(12, 6), nullable=True)
    error_message = Column(Text, nullable=True)
    refunded = Column(Boolean, nullable=False, server_default='false', default=False)
    created_at = Column(DateTime(timezone=True), nullable=False, server_default=func.now())
    updated_at = Column(DateTime(timezone=True), nullable=False, server_default=func.now(), onupdate=func.now())
...
async def register_models():
    async with Database().engine.begin() as conn:
        await conn.run_sync(Database.BASE.metadata.create_all, checkfirst=True)
